...
std::ostream& operator<<(std::ostream& os, const Card& card) {
    // Если карта закрыта, выводим заглушку
    if (!card.getFaceUp()) {
        os << "??";
        return os;
    }

    // Словари для преобразования номинала и масти в строковое представление
    ...
    // Вывод номинала и масти карты
    os << rankNames[card.getRank()] << suitSymbols[card.getSuit()];
    return os;
}
...
protected:
    ...
    void createDecks() {
        cards.clear();  // Очищаем текущую колоду

        // Создание указанного числа колод
        for (int d = 0; d < numDecks; ++d) {
            // Перебираем все масти
            for (int s = 3; s <= 6; ++s) {
                // Перебираем все значения карт
                for (int r = 2; r <= 14; ++r) {
                    cards.emplace_back(
                        // Преобразуем значения для корректной игры
                        static_cast<Rank>(r <= 10 ? r : (r == 14 ? 11 : 10)),
                        static_cast<Suit>(s)
                    );
                }
            }
        }
        shuffle();  // Перемешиваем колоду после создания
    }
...
public:
    ...
    // Виртуальный метод обновления очков с учетом правил BlackJack
    virtual void updateScore() {
        score = 0;
        int aceCount = 0;

        // Подсчет очков с особой логикой для тузов
        ...
        // Корректировка очков, если сумма больше 21
        while (score > 21 && aceCount > 0) {
            score -= 10;  // Туз может считаться как 1 вместо 11
            aceCount--;
        }
